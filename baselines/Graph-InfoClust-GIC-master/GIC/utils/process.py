...
def mask_test_edges(adj, test_frac=.1, val_frac=.05, prevent_disconnect=True, verbose=False):
    ...
    while len(test_edges_false) < num_test*3:          # 反例的数量是正例的3倍     
    ...
    while len(val_edges_false) < num_val*3:       # 反例的数量是正例的3倍
        idx_i = np.random.randint(0, adj.shape[0])
        idx_j = np.random.randint(0, adj.shape[0])
        if idx_i == idx_j:
            continue
    ...
    while len(train_edges_false) < len(train_edges)*3:        # 反例的数量是正例的3倍
        idx_i = np.random.randint(0, adj.shape[0])
        idx_j = np.random.randint(0, adj.shape[0])
        if idx_i == idx_j:
            continue
